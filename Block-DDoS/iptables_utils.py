...
def block_ip(ip, reason):
    ...
    max_retries = 3  # Maximum number of retries for blocking an IP in case of I/O error
        ...
        if ip != "127.0.0.1":
            ...
            while retries < max_retries:
                try:
                    ...
                    result = subprocess.run(f"conntrack -D -d {ip}", shell=True, stderr=subprocess.PIPE)
                    if result.returncode != 0:
                        print(f"{Colors.YELLOW}No established connections found for destination IP {ip}. Skipping...{Colors.RESET}")
                    else:
                        print(f"{Colors.YELLOW}Dropped established connections for IP {ip}{Colors.RESET}")
                    break
                except subprocess.CalledProcessError as e:
                    retries += 1
                    print(f"{Colors.RED}Error blocking IP {ip}, retry {retries}/{max_retries}: {str(e)}{Colors.RESET}")
                    time.sleep(retry_delay)
                    if retries >= max_retries:
                        log_error(f"Error blocking IP {ip}: {str(e)}")
                        print(f"{Colors.RED}Failed to block IP {ip} after {max_retries} attempts.{Colors.RESET}")
                        break
    except Exception as e:
        log_error(f"Error blocking IP {ip}: {str(e)}")
